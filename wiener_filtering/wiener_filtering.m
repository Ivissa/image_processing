% ----------------对灰度图像进行运动模糊并叠加高斯噪声，并采用维纳滤波进行复原-------------
clc;         %清空控制台
clear;       %清空工作区
close all;   %关闭已打开的figure图像窗口
color_pic=imread('test.jpg');  %读取彩色图像
...
figure('name','维纳滤波结果');
subplot(1,3,1);imshow(im2uint8(restore_ignore_noise),[]);title('信噪比未知');
subplot(1,3,2);imshow(im2uint8(restore_with_noise),[]);title('信噪比已知');
...
subplot(1,3,3);imshow(restore_with_noise,[]);title('图像和噪声自相关函数已知')
